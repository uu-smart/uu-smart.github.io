//#region src/client/composables/decrypt.ts
/**
...
function useDecrypt(config) {
	...
	*/
	const toUnit8Array = (raw) => Uint8Array.from(raw);
	return { decrypt: async (password, text) => {
		if (!password) return;
		const key = await getCryptoDeriveKey(await getKeyMaterial(password), toUnit8Array(config.value.salt));
