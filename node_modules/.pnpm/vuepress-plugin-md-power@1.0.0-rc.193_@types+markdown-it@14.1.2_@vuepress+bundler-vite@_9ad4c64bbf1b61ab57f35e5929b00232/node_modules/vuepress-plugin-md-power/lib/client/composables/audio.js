...
function useAudioPlayer(source, options = {}) {
	...
	function isSupportType() {
		...
		if (!isSupported) console.warn(`The specified type "${type}" is not supported by the browser.`);
		return isSupported;
	}
	...
	function getLastBufferedTime() {
		...
		const buff = bufferedRanges.find((buff) => buff.start < player.currentTime && buff.end > player.currentTime);
		if (buff) return buff.end;
		return bufferedRanges[bufferedRanges.length - 1].end;
	}
	function isValidDuration(duration) {
		if (duration && !Number.isNaN(duration) && duration !== Number.POSITIVE_INFINITY && duration !== Number.NEGATIVE_INFINITY) return true;
		return false;
	}
	function destroy() {
		player?.pause();
